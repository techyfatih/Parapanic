...
namespace Parapanic
{
    ...
    class World
    {
        ...
        public World(int width, int height)
        {
            ...
            foreach (Vector2[] connection in connections)
            {
                ...
                if (horizontalFirst)
                {
                    if (deltaX > 0)
                    {
                        for (int x = xi; x <= xf; x++)
                            if (grid[x, yi] == null)
                                grid[x, yi] = new Road(x * Block.size, yi * Block.size);
                    }
                    else
                    {
                        for (int x = xi; x >= xf; x--)
                    ...
                    if (deltaY > 0)
                    {
                        for (int y = yi; y <= yf; y++)
                            if (grid[xf, y] == null)
                                grid[xf, y] = new Road(xf * Block.size, y * Block.size);
                    }
                    else
                    {
                        for (int y = yi; y >= yf; y--)
            ...
            for (int i = BORDERWIDTH; i < width - BORDERWIDTH; i++)
            {
                for (int u = BORDERHEIGHT; u < height - BORDERHEIGHT; u++)
                {
                    ...
                    {
                        grid[i, u] = new Road(i * Block.size, u * Block.size);
                    }
                    else
                    {
                        grid[i, u] = new WallBlock(i * Block.size, u * Block.size);
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++ )
                {
                    if (x == 0 || x == width - 1 || y == 0 || y == height - 1)
                        ...
                        grid[x, y] = new Road(x * Block.size, y * Block.size);
                }
            }*/
        ...
        public Vector2 EmptySpace()
        {
            ...
            foreach (Block b in grid)
                if (b.GetType().Equals(typeof(Road))) empties.Add(b.position);
